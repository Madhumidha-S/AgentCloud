...
def _reset_sqlite(path: Path | str) -> None:
    ...
    if p.exists():
        p.unlink()
    # create empty file and schema lazily in MemoryAgent
...
def run_episode(
    *,
    ...
    # Use simulator directly (no threads) and disable sleeps for fast evaluation.
    events = generate_events(scenario, tick_seconds=0.0)
    ...
    memory = MemoryAgent(sqlite_path=settings.sqlite_path)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    summary = {
        ...
        "success_rate": (total_success / total_incidents) if total_incidents else 0.0,
    ...
    if args.json:
        ...
        if args.ablation and not args.json:
            ...
            nm_succ = (
                sum(r.success_count for r in nm) / max(1, sum(r.incidents_handled for r in nm))
            )
            np = _run(bool(args.no_memory), True)
            np_succ = (
                sum(r.success_count for r in np) / max(1, sum(r.incidents_handled for r in np))
            )
            print(f"Full System → Success: {round(base * 100)}%")
            print(f"No Memory → Success: {round(nm_succ * 100)}%")
            print(f"No Planning → Success: {round(np_succ * 100)}%")
